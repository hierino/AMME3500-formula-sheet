...
\sub{Observability} System observable $\Leftrightarrow \det[C;CA;CA^2]\neq0$ $\Leftrightarrow$ can uniquely recover $\mathbf x(0)$ from measured $u,y$ over $[0,T]$.\\
\sub{Observer purpose} estimate unmeasured state $\mathbf x$ from available $u,y$ → enables state-feedback when full state unavailable. Works iff $A-LC$ stable (error $\tilde{\mathbf x}\to0$).\\
