...
\usepackage[a4paper,landscape, margin = 6mm]{geometry} %Original margin = 6mm;  left=5mm, right=5mm, top = 5mm, bottom = 6mm
\usepackage{multicol}
...
Step ($u{=}$const), $x(0){=}x_0$:$\qquad$ (don't forget step $u\cdot\mathbf{1}(t)$)
...
Plant $\dot x+ax=u$, $u=K_P(r-x)\quad$  (state-feedback controller):
\[ \dot x+(a{+}K_P)x=K_Pr \]
...
\sub{Reachability matrix} (3rd order: $3\times3$)
\[ W=\begin{bmatrix}B&AB&A^2B\end{bmatrix} \]
Reachable $\Leftrightarrow \det W\neq0$.\\
Equivalent: can place \emph{any} closed-loop poles via $K$, for $K=\begin{pmatrix}k_1&k_2\end{pmatrix}$.\\
...
\sub{Observability matrix} (3rd order: $3\times3$)
...
\sub{Separation principle} systems \emph{both reachable \& observable}: design cont $K$ and obsv $L$ \textbf{independently}; the CL poles are just the union of the controller poles (eig $A{-}BK$) and observer poles (eig $A{-}LC$).\\
%  applies when system is both reachable \& observable: design controller $K$ and observer $L$ independently; CL poles = union of $(A-BK)$ poles and $(A-LC)$ poles.
...
\sub{Forced sinusoid (undetermined coeffs)} for $\ddot x+\omega_n^2x=F\sin\omega t$, try $x{=}X\sin\omega t$, match:
...
% Start height (flat, low $\omega$) $=20\log_{10}K$ dB. If sloped (i.e pole @ 0), choose small $\omega$ and S.H. $=20\log_{10}\left(\frac{K}{\omega}\right)\Rightarrow K=\omega\cdot 10^{\mathrm{S.H}/20}$.\\
\sub{Start height} $H(j\omega)_{\mathrm{dB}}=20\log_{10}(K\omega)=20\log_{10}(K)+20\log_{10}(\omega)$\\
...
\sub{Time delay} Max delay $\tau_{max}=\varphi_m/\omega_{gc}$.
...
\sub{Loop shaping — why} shape $L{=}PC$ to balance performance vs stability across frequency. Can't have both $S$ and $T$ low everywhere, shapre $L$ to benefit where it matters. \\
\sub{how} Tune $C$ (adding gain, poles or zeros) s.t. (i) high gain / integrator at low $\omega$ $\to$ tracking + reject $d$ ($S{\approx}0$); (ii) low gain + steep roll-off at high $\omega$ $\to$ reject noise $n$ ($T{\approx}0$); (iii) gentle slope ($\approx{-}20$dB/dec) near $\omega_{gc}$ $\to$ keep phase margin.\\
...
$L(s)=C(s)P(s)$ (loop TF), $T(s)=\tfrac{L(s)}{1+L(s)}$, $S(s)=\tfrac{1}{1+L(s)}$ (sensitivity function measures rel change of CL TF wrt to small change of the plant).\\
\sub{Cascaded SS — recipe} EOM of each subsystem:
\[ \dot x_1{=}A_1x_1{+}B_1u_1,\ y_1{=}C_1x_1;\quad \dot x_2{=}A_2x_2{+}B_2u_2,\ y_2{=}C_2x_2 \]
Sub interconnection $u_2{=}y_1{=}C_1x_1$ into $\dot x_2$; stack $x{=}[x_1;x_2]$:
\[ \dot x=\begin{bmatrix}A_1&0\\B_2C_1&A_2\end{bmatrix}x+\begin{bmatrix}B_1\\0\end{bmatrix}u_1,\quad y_2=\begin{bmatrix}0&C_2\end{bmatrix}x \]
% ($A$ block lower-triangular $\Rightarrow$ eig$(A)=$ eig$(A_1)\cup$ eig$(A_2)$ — quick pole check).\\
% \sub{Reachability/observability of $S$} test full $W{=}[B\ AB\ \cdots]$, $W_o{=}[C;CA;\cdots]$ on the \emph{combined} system — \textbf{not} automatic even if $S_1,S_2$ individually are (pole-zero cancellation between $G_1,G_2$ can hide a mode).\\
\sub{TF} get $G_i{=}C_i(sI{-}A_i)^{-1}B_i{+}D_i$ for each subsystem, then $G_{tot}{=}G_2G_1$ (multiply) — far easier than inverting the big $(sI{-}A)$.
...
\item Real-world uncertainty: parameter errors, disturbances, sensor noise, noise affecting dynamics, unmodelled nonlinearity.
\end{itemize}
\sub{PID vs state-feedback} \textbf{PID}: model-free (acts on error), mainly SISO, heuristic/robust, simple to tune, limited pole control. \textbf{State-FB}: needs accurate model + full state (or observer), handles MIMO, gives exact pole placement.\\
...
% ---------------------------------------------------
\sect{21. Written Question Framework}
% \sub{Stability} System stable $\Leftrightarrow$ all poles/eigenvalues have negative real part $\Leftrightarrow$ free response $x(t)\!\to\!0$ for any $x(0)$, zero input.\\
\sub{Observability} Can uniquely recover initial value $\mathbf x(0)$ from measured $u,y$ over $[0,T]$. Find unmeausrable from what you can measure.\\
\sub{Observer purpose} estimate unmeasured state $\mathbf x$ from available $u,y$ → enables state-feedback when full state unavailable. Asymptotically tracks state of the plant.\\
% Works iff $A-LC$ stable (error $\tilde{\mathbf x}\to0$).\\
% \sub{Observer gain L condition} eigenvalues (roots) of $A-LC$ must be negative real (or stable complex, real part $<0$) so observer error decays to zero.\\
\sub{Sensitivity function $S$} measures system sensitivity to plant uncertainty; $|S|\ll1$ means disturbances attenuated (good tracking, disturbance rejection). $S{+}T=1$ (seesaw): cannot make both small.\\
\sub{$S$ \& $T$ trade-off} $S$ low at low $\omega$ (track, reject slow disturbances); $T$ low at high $\omega$ (reject fast noise). Near crossover $\omega_{gc}$, phase margin enforces balance.\\
% \sub{Separation principle} applies when system is both reachable \& observable: design controller $K$ and observer $L$ independently; CL poles = union of $(A-BK)$ poles and $(A-LC)$ poles.\\
% \sub{PID vs state-feedback} \textbf{PID}: model-free, robust, SISO only, heuristic tuning, limited pole placement. \textbf{State-FB}: requires model + full state, MIMO capable, arbitrary pole placement, sensitive to model errors.\\
% \sub{Nyquist stability} plot $L(j\omega)$ for $\omega:-\infty\to\infty$. Closed loop stable iff Nyquist does \emph{not} encircle $-1$ (count: \# clockwise = $Z-P$, need $Z=0$).\\
\sub{Gain margin} the factor by which the loop gain can be increased before closed-loop instability occurs. Larger GM $\Rightarrow$ more robust to gain errors/uncertainty.\\
\sub{Real-world control design} (1) model system (ODE/SS from physics); (2) ID parameters (experiments, data); (3) check reachable/observable; (4) design $K$ for transient specs ($\zeta,\omega_n$); (5) design $L$ for measurement noise tolerance; (6) check closed-loop stability (Nyquist/poles); (7) simulate with real noise/uncertainty.\\
% \sub{Real-world uncertainties} parameter errors (measured $\neq$ true), disturbances (wind, friction, etc.), sensor noise, unmodelled dynamics (neglected high-freq poles/delays), nonlinearity (saturation, friction).\\
\sub{Step response features from TF} $G(s)$ poles $\Rightarrow$ poles in TF $\Rightarrow$ envelope decay $e^{-\sigma t}$ and oscillation frequency $\omega_d$. Final value $x(\infty)=G(0)\cdot U$. DC gain sign $\Rightarrow$ direction (dip/rise).\\
% \sub{Bode plot interpretation} (1) flat low-$\omega$ $\Rightarrow$ DC gain, slope $-20$/dec per integrator. (2) Corners = pole ($-20$), zero ($+20$), or complex pair ($\mp40$). (3) Phase: flat $\to -90^\circ$ at each pole. (4) Count integrators from initial slope \& final phase. (5) Magnitude @ $0$dB gives crossover; phase there $\Rightarrow$ phase margin.

% ---------------------------------------------------
\sect{22. Math Toolbox}
