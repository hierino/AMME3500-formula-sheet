...
\sub{Gain margin} $GM=1/|L(j\omega_{pc})|$ where phase $=-180°$. On Nyquist, find where curve crosses negative real axis; GM = reciprocal of magnitude. On Bode, read magnitude (dB) at phase $-180°$.\\
...
\sub{Bode plot interpretation} (1) flat low-$\omega$ $\Rightarrow$ DC gain, slope $-20$/dec per integrator. (2) Corners = pole ($-20$), zero ($+20$), or complex pair ($\mp40$). (3) Phase: flat $\to -90$ → flat at each pole. (4) Count integrators from initial slope \& final phase. (5) Magnitude @ $0$dB gives crossover; phase there $\Rightarrow$ phase margin.
}
